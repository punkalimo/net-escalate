...
const interfaceMetricsSchema = new mongoose.Schema(
  {
    ...
    speedSource: { type: String, default: null },
...
const snmpSchema = new mongoose.Schema(
  {
    ...
    securityLevel: { type: String, enum: ["noAuthNoPriv", "authNoPriv", "authPriv"], default: "noAuthNoPriv" },
...
const deviceSchema = new mongoose.Schema(
  {
    ...
    ipAddress: { type: String, unique: true, required: true, trim: true },
    deviceType: { type: String, enum: ["router", "switch", "firewall", "server", "access-point", "printer", "other"], default: "other" },
    ...
    pollingInterval: { type: Number, default: 30, min: 5 },
    monitoringMethods: { type: [{ type: String, enum: ["icmp", "snmp", "http", "https"] }], default: ["icmp"] },
    snmp: { type: snmpSchema, default: () => ({}) },
    http: {
      enabled: { type: Boolean, default: false },
    ...
    monitoredPorts: { type: [monitoredPortSchema], default: [] },
