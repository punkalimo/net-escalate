...
export default function phase4Routes(io) {
  ...
  router.get("/overview", async (req, res) => {
    try {
      const [devices, active, recent, samples] = await Promise.all([Device.find({ realmId: req.realmId }).lean(), Incident.countDocuments({ realmId: req.realmId, status: { $nin: ["RESOLVED"] } }), Incident.find({ realmId: req.realmId }).sort({ createdAt: -1 }).limit(50).lean(), InterfaceSample.find({ realmId: req.realmId }).sort({ sampledAt: -1 }).limit(500).lean()]);
  ...
  router.get("/analytics", async (req, res) => {
    try {
      ...
      const [incidents, samples] = await Promise.all([Incident.find({ realmId: req.realmId, createdAt: { $gte: since } }).sort({ createdAt: 1 }).lean(), InterfaceSample.find({ realmId: req.realmId, sampledAt: { $gte: since } }).sort({ sampledAt: 1 }).lean()]);
  ...
  router.get("/rca", async (req, res) => {
    try {
      const correlation = await correlateActiveIncidents({ realmId: req.realmId, forceTopology: req.query.refresh === "true" });
  ...
  router.get("/config-changes", async (req, res) => {
    try {
      const snapshots = await ConfigSnapshot.find({ realmId: req.realmId }).sort({ capturedAt: -1 }).limit(200).lean();
  ...
  router.post("/config-snapshots/:deviceId", async (req, res) => {
    try { const device = await Device.findOne({ deviceId: req.params.deviceId, realmId: req.realmId }); if (!device) return res.status(404).json({ success: false, message: "Device not found." }); return res.status(201).json({ success: true, snapshot: await captureSnapshot(device) }); }
  ...
  router.post("/automation/run", async (req, res) => {
    ...
    try {
      if (action === "correlate") return res.json({ success: true, action, result: await correlateActiveIncidents({ realmId: req.realmId, forceTopology: true }) });
      if (action === "topology") return res.json({ success: true, action, result: await discoverTopology(req.realmId) });
      if (action === "snapshot" && deviceId) { const device = await Device.findOne({ deviceId, realmId: req.realmId }); if (!device) return res.status(404).json({ success: false, message: "Device not found." }); return res.json({ success: true, action, result: await captureSnapshot(device) }); }
      if (action === "interfaces" && deviceId) { const device = await Device.findOne({ deviceId, realmId: req.realmId }).lean(); if (!device) return res.status(404).json({ success: false, message: "Device not found." }); if (!device.snmp?.enabled) return res.status(400).json({ success: false, message: "SNMP is disabled for this device." }); return res.json({ success: true, action, result: await discoverInterfaces(device) }); }
  ...
  router.post("/assistant", async (req, res) => {
    try {
      ...
      if (incidentId) {
        const scoped = await answerIncidentQuestion(question, incidentId, req.realmId);
        if (scoped.success) return res.json(scoped);
      }

      const [devices, incidents, samples] = await Promise.all([Device.find({ realmId: req.realmId }).lean(), Incident.find({ realmId: req.realmId, status: { $ne: "RESOLVED" } }).sort({ createdAt: -1 }).limit(100).lean(), InterfaceSample.find({ realmId: req.realmId }).sort({ sampledAt: -1 }).limit(100).lean()]);
