...
import {
  ...
  Monitor, AlertTriangle, CheckCircle2, Cable, Route, ScanLine, Gauge,
  CircleDot, ExternalLink, Activity, Clock3
} from "lucide-react";
...
function PathCanvas({ path, selectedId, setSelectedId }) {
  ...
  return <div className="relative min-h-[560px] overflow-auto rounded-2xl border border-slate-800 bg-[#050a12]" style={{backgroundImage:"radial-gradient(circle at 50% 35%, rgba(14,165,233,.12), transparent 38%), linear-gradient(rgba(51,65,85,.10) 1px, transparent 1px), linear-gradient(90deg, rgba(51,65,85,.10) 1px, transparent 1px)",backgroundSize:"auto, 32px 32px, 32px 32px"}}>
    <svg viewBox={`0 0 ${width} 560`} className="min-h-[560px] min-w-[1100px] w-full">
      ...
      <div className="mx-auto flex h-full max-w-[1850px] flex-col overflow-hidden rounded-2xl border border-slate-800 bg-[#070c15] shadow-2xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
          ...
          <main className="min-h-0 flex-1 overflow-auto p-4">
            {mode === "fleet" ? <div style={{transform:`scale(${zoom})`,transformOrigin:"top left",width:`${100/zoom}%`}}>{loading && !filtered ? <div className="flex min-h-[620px] items-center justify-center text-sm text-slate-500"><RefreshCw className="mr-2 animate-spin" size={17}/>Discovering topology via SNMP...</div> : filtered ? <TopologyCanvas topology={filtered} selectedId={selectedId} setSelectedId={setSelectedId}/> : <div className="flex min-h-[620px] items-center justify-center text-slate-600">No topology data.</div>}</div> : <div className="space-y-4">
