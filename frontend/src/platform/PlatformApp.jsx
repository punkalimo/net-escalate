...
import { LayoutDashboard, Building2, UserRound, Server, AlertTriangle, BarChart3, ScrollText, Network, LogOut } from "lucide-react";
...
function Shell({ user, onLoggedOut, children }) {
    ...
    <aside className="fixed inset-y-0 left-0 z-40 w-64 border-r border-slate-800/80 bg-[#080d16]/95 backdrop-blur-xl">
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-slate-800/80 p-4">
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
          ...
          <button onClick={doLogout} className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-slate-700 px-2 py-1.5 text-[11px] text-slate-400 hover:bg-slate-800 hover:text-white"><LogOut size={12} />Sign out</button>
        </div>
      </div>
    </aside>
...
export default function PlatformApp({ user, onLoggedOut }) {
  return <BrowserRouter>
    <Shell user={user} onLoggedOut={onLoggedOut}>
      <Routes>
